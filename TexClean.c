...
int main(int argc,char** argv){
    ...
    //Mode récursif
        ...
        lua_pushstring(L,*(argv+2));
        lua_pushboolean(L,1);
    }else if(argc == 2 && !strcmp(*(argv+1),"makeRec")){ //On créé les pdf du dossier courant
        lua_getglobal(L,"makeRec");
        lua_pushstring(L,".");
        lua_pushboolean(L,1);
    }else if(argc==3 && !strcmp(*(argv+1),"makeRec") && gFS_isDir(*(argv+2)) ){ //On créé les pdf d'un dossier donné
        lua_getglobal(L,"makeRec");
        lua_pushstring(L,*(argv+2));
        lua_pushboolean(L,0);
